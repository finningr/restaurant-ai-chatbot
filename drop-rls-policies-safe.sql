-- Drop RLS policies and disable RLS on tables EXCEPT chat_messages (leave it alone)
...
-- Now disable RLS on all tables EXCEPT chat_messages (leave it alone since it's working)
...
-- Verify: Check RLS status and remaining policies
SELECT 
  t.tablename,
  CASE 
